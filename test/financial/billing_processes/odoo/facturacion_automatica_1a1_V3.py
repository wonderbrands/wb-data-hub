...
ERROR_502_COUNTER = 0  # Contador global para el monitoreo de caídas 502
...
def reset_stuck_processing_records():
    """
    Si una corrida anterior murió a la mitad (por Bad Gateway 502, kill -9, caída de red, etc.),
    los registros se quedan trabados en 'PROCESSING' para siempre. Esta limpieza inicial
    los regresa a 'ERROR' para que puedan ser retomados por el flujo de resiliencia.
    """
    conn = get_db_connection()
    if not conn: return
    try:
        ...
        conn.commit()
        if affected:
            logging.warning(f"Limpieza inicial: {affected} registro(s) trabados en 'PROCESSING' regresados a 'ERROR' para reintento.")
    except Error as e:
        logging.error(f"No se pudo ejecutar la limpieza de registros 'PROCESSING': {e}")
    finally:
        if conn.is_connected():
            cursor.close()
            conn.close()
...
def get_failed_order_ids():
    """
    Obtiene los IDs de órdenes de venta en Odoo que tuvieron error en ejecuciones previas
    (ej. se quedaron en borrador o confirmadas sin timbrar por un 502 Bad Gateway) y que son
    candidatas a reanudar el proceso. Se limita la ventana de tiempo a 30 días y máximo 5 intentos.
    """
    conn = get_db_connection()
    if not conn: return []
    try:
        ...
        logging.error(f"Error consultando órdenes fallidas en BD: {e}")
        return []
    finally:
        if conn.is_connected():
            cursor.close()
...
class TimeoutTransport(xmlrpc.client.SafeTransport):
    """Transporte personalizado para forzar un timeout en XML-RPC sobre HTTPS."""
    ...
    def __init__(self, url, db, user, pwd, timeout=300, max_retries_init=3, delay_init=2):
        ...
        for attempt in range(1, self.max_retries_init + 1):
            try:
                ...
                logging.info(f"Conectado a Odoo correctamente (intento {attempt}).")
                return  
            except (xmlrpc.client.ProtocolError, TimeoutError, OSError, ConnectionError) as e:
                logging.warning(f"Error de red/Timeout en conexión inicial (intento {attempt}/{self.max_retries_init}): {e}")
                ERROR_502_COUNTER += 1
                if attempt == self.max_retries_init:
                    raise  
                tm.sleep(self.delay_init * attempt)
            except Exception as e:
                logging.error(f"Error fatal en conexión inicial (no reintentable): {e}")
                raise

    def reauthenticate(self):
        logging.info("Cerrando sesión TLS y abriendo una nueva conexión con Odoo...")
        transport = TimeoutTransport(timeout=self.timeout)
    ...
    def execute_kw(self, db, uid, pwd, model, method, args, kwargs=None, max_retries=3, delay=2):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                ERROR_502_COUNTER += 1
                logging.warning(f"Error de red/Timeout: {e.errcode if hasattr(e, 'errcode') else ''} / {e.errmsg if hasattr(e, 'errmsg') else ''} en Odoo [{model}.{method}]: {str(e)}. Intento {attempt}/{max_retries}...")
                ...
                try:
                    ...
                    logging.warning(f"Error al reautenticar con Odoo: {str(auth_e)}")
            except Exception as e:
                logging.warning(f"Error de comunicación en Odoo [{model}.{method}]. Intento {attempt}/{max_retries}: {str(e)}")
                ERROR_502_COUNTER += 1
                if attempt == max_retries:
                    raise e
                tm.sleep(delay * attempt)
...
def main():
    conections_count = 0
    reset_stuck_processing_records()  # Limpiamos procesos trabados por caídas abruptas anteriores
    while True:
        try:
            run()
            break  
...
def run():
    ...
    # Reemplazo de ServerProxy crudos por el OdooModelProxy con resiliencia 502
    models = OdooModelProxy(server_url, db_name, username, password, timeout=300)
    uid = models.uid
    logging.info('Conexion con Odoo establecida (via OdooModelProxy)')
    ...
    # --- Recuperación de órdenes en estatus ERROR para reanudar su facturación incompleta ---
    ...
    if failed_ids:
        ...
        if missing_failed_ids:
            logging.info(f"Recuperando {len(missing_failed_ids)} ordenes con errores previos en BD para reanudar su facturacion...")
            for chunk in get_chunks(missing_failed_ids, 500):
                try:
                    failed_records = models.execute_kw(db_name, uid, password, 'sale.order', 'search_read', [[('id', 'in', chunk)]])
                    all_records.extend(failed_records)
                except Exception as e:
                    logging.error(f"Error extrayendo ordenes fallidas: {e}")
    ...
    logging.info(f"ERROR_502_COUNTER FINAL: {ERROR_502_COUNTER}")
...
def process_records(records, delta_days, failed_ids=None):
    ...
    for record in records:
        # Permitimos el paso si está "to invoice" O si es una orden en reintento de resiliencia
...
def execute_invoice(team_name, orders_list):
    ...
    # Consultamos estado y UUID SAT de facturas existentes para reanudación granular
    ...
    for index, order in enumerate(orders_list): 
        ...
        # --- LÓGICA DE REANUDACIÓN GRANULAR ANTE ERRORES Y CAÍDAS DE ODOO ---
        if order_name in invoiced_origins:
            ...
            if inv_state == 'posted' and is_stamped:
                logging.warning(f"BUCLE EVITADO: {order_name} YA TIENE la factura timbrada {inv_name} (UUID: {inv_uuid}). Se ignorara.")
                ...
                logging.info(f"Reanudando orden {order_name}: Factura {inv_name} (ID: {inv_id}) ya confirmada. Continuando únicamente con timbrado SAT...")
                real_invoice_name = inv_name
                needs_creation = False
                needs_post = False
                needs_stamping = True
            elif inv_state == 'draft' or inv_name in (False, 'False'):
                logging.info(f"Reanudando orden {order_name}: Factura en borrador (ID: {inv_id}) encontrada. Continuando desde confirmación (action_post)...")
                needs_creation = False
                needs_post = True
                needs_stamping = True
            else:
                logging.warning(f"BUCLE EVITADO: {order_name} YA TIENE la factura {inv_name} en estado {inv_state}. Se ignorara.")
        ...
        if (order['state'] == 'sale' and order['locked']) or (order_name in orders_list_not_serialize_message):
            if (order['invoice_status'] == 'to invoice' and order['invoice_count'] == 0) or not needs_creation:
                try:
                    if needs_creation:
                        ...
                        if invoice_line_vals_list and not abortar_orden:
                            ...
                            try:
                                ...
                                logging.error(f"Error al CREAR factura {order_name}: {e_create}")
                    ...
                    # --- PASO DE CONFIRMACIÓN (action_post) ---
                    if needs_post and inv_id:
                        try:
                            if needs_creation and acc_cxc_team:
                                ...
                                if lines_to_update:
                                    models.execute_kw(db_name, uid, password, 'account.move', 'write', [[inv_id], {'line_ids': lines_to_update}])
                            
                            if needs_creation:
                                models.execute_kw(db_name, uid, password, 'account.move', 'message_post', [inv_id], {'body': f'Factura 1 a 1 para {order_name}. Creada via API.', 'message_type': 'comment'})
                            
                            models.execute_kw(db_name, uid, password, 'account.move', 'action_post', [[inv_id]])    
                            
                            inv_data = models.execute_kw(db_name, uid, password, 'account.move', 'read', [[inv_id]], {'fields': ['name']})
                            real_invoice_name = inv_data[0]['name'] if inv_data else str(inv_id)
                            
                        except xmlrpc.client.Fault as e_post:
                            logging.error(f"Fallo al CONFIRMAR factura {order_name}: {e_post.faultString}")
                            ...
                            logging.error(f"Error al CONFIRMAR factura {order_name}: {e_post}")
                    ...
                    if needs_stamping and inv_id:
                        try:
                            ...
                            logging.debug(f"[{success_count}/{total_orders}] Factura individual {real_invoice_name} creada/reanudada y timbrada para {order_name}")
                            tm.sleep(0.3)
                            ...
                            logging.error(f"Error al TIMBRAR factura de {order_name}: {e_stamp}")
