    ...
    if ready_to_post_list:
        ...
        while not post_success:
            try:
                ...
                error_str = str(ex)
                if "502" in error_str or "bad gateway" in error_str.lower() or ...:
                    log.warning("Error 502 en action_post. Reintentando después de renovar conexión...")
                    # Renovar el proxy manualmente
                    models.reauthenticate()
                    # Opcional: esperar un poco más
                    time.sleep(2)
                    continue
